...
import 'package:notes_app/repositories/notes/notes_repository.dart';
...
class NoteDetailBloc extends Bloc<NoteDetailEvent, NoteDetailState> {
  ...
  Stream<NoteDetailState> _mapNoteSavedToState() async* {
    ...
    try {
      ...
      yield NoteDetailState.failure(
          note: state.note, errorMessage: 'Note could not be saved');
      yield state.update(
        isSubmitting: false,
        isSuccess: false,
        isFailure: false,
  ...
  Stream<NoteDetailState> _mapNoteDeletedToState() async* {
    ...
    try {
      ...
      yield NoteDetailState.failure(
          note: state.note, errorMessage: 'Note could not be deleted');
      yield state.update(
        isSubmitting: false,
        isSuccess: false,
        isFailure: false,
